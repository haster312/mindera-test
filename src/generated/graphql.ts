...
export type ProductQueryQuery = { products: { edges: Array<{ cursor: string, node: { id: string, title: string, description: string, availableForSale: boolean, images: { edges: Array<{ node: { src: any, altText?: string | null } }> }, variants: { edges: Array<{ node: { price: { amount: any, currencyCode: CurrencyCode }, compareAtPrice?: { amount: any, currencyCode: CurrencyCode } | null } }> } } }>, pageInfo: { hasNextPage: boolean, endCursor?: string | null } } };


export const ProductQueryDocument = gql`
...
const ProductQueryDocumentString = print(ProductQueryDocument);
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    ProductQuery(variables: ProductQueryQueryVariables, requestHeaders?: GraphQLClientRequestHeaders): Promise<{ data: ProductQueryQuery; errors?: GraphQLError[]; extensions?: any; headers: Headers; status: number; }> {
        return withWrapper((wrappedRequestHeaders) => client.rawRequest<ProductQueryQuery>(ProductQueryDocumentString, variables, {...requestHeaders, ...wrappedRequestHeaders}), 'ProductQuery', 'query', variables);
    }
  };
}
